...
// on construit un index par salle/jour/heure pour le JS
$jauge_index = [];
        ...
        <form action="reservation.php" method="POST" id="formReservation">
            ...
            <div class="form-section-linear">
                ...
                <div class="radio-group-horizontal">
                    ...
                    <label class="custom-radio">
                        <input type="radio" name="id_categorie" value="<?= $cat['id_categorie'] ?>" <?= $index === 0 ? 'checked' : '' ?>>
                        <span class="radio-mark"></span> <?= htmlspecialchars($cat['libelle']) ?>
                    </label>
                    <?php endforeach; ?>
                </div>
            </div>
            ...
            <div class="form-section-linear">
                ...
                <div class="salles-accordion-list">
                    ...
                    ?>
                    <details class="salle-accordion" data-salle-id="<?= $id_salle ?>">
                        <summary class="salle-summary">
                            <div class="salle-summary-text">
                                <h4><?= htmlspecialchars($salle['nom']) ?></h4>
                                <small>Capacité maximale : <?= $salle['capacite_max'] ?> personnes</small>
                            </div>
                            <span class="accordion-arrow">▼</span>
                        </summary>
                        <div class="horaires-dropdown-content">
                            <input type="hidden" name="creneaux[<?= $id_salle ?>]" id="hidden_id_creneau_<?= $id_salle ?>" value="">
                            <div class="creneaux-zone" id="creneaux_salle_<?= $id_salle ?>">
                                <p class="creneaux-hint">Ouvrez cette salle pour voir les créneaux disponibles.</p>
                            </div>
                            <div class="nb-personnes-zone" id="nb_zone_<?= $id_salle ?>" style="display:none; margin-top: 20px;">
                                <div class="input-group" style="max-width: 250px;">
                                    <label for="nb_personnes_<?= $id_salle ?>">Nombre de personnes <span class="red-dot">*</span></label>
                                    <input type="number" id="nb_personnes_<?= $id_salle ?>" name="nb_personnes[<?= $id_salle ?>]" min="1" max="12" value="1">
                                    <small id="jauge-info-<?= $id_salle ?>" class="jauge-info-txt"></small>
                                </div>
                            </div>
                        </div>
                    </details>
...
function renderCreneaux(salleId, jourKey) {
    ...
    const jaugesSalle = JAUGE_INDEX[salleId]?.[jourKey] ?? {};
    ...
    // remet la sélection en évidence si l'utilisateur avait déjà choisi un créneau
    const prev = selectedBySalle[salleId];
    if (prev) {
        zone.querySelectorAll('.horaire-badge-item').forEach(b => {
    ...
    zone.querySelectorAll('.horaire-badge-item').forEach(badge => {
        badge.addEventListener('click', function() {
            ...
            thisZone.querySelectorAll('.horaire-badge-item').forEach(b => b.classList.remove('selected'));
...
document.getElementById('formReservation').addEventListener('submit', function(e) {
    ...
    const valide = Array.from(inputs).some(input => input.value !== '');
    if (!valide) {
        e.preventDefault();
